...
int main()
{
    ...
    key = SHMKEY;
    size_t size = 17*sizeof(SharedData);
    ...
    for(int i = 0;i<16;i++)
    {
        if((shared_data+i)->ready == 1)
        {
            strcpy(MATRIX1,(shared_data+i)->matrix1);
            strcpy(MATRIX2,(shared_data+i)->matrix2);
            strcpy(OP,(shared_data+i)->op);
            run();
            (shared_data+i)->ready=0;
        }
    }

    if(shared_data[16].ready == -2)
    {
